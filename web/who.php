...
foreach ($va_display_order as $va_types) {
    ...
    if ($rep_count > 1) {
        ...
        if (count($representatives[0]) && $representatives_info[$representatives[0][0]]['name'] == 'Vacant Seat') {
            $text = _("<p>There’s an upcoming election.  We’ll be adding your new representative as soon as we can after the election.</p>");
            $col_after = '';
...
function extra_mp_text($rep_count, $va_area, $representatives) {
    ...
    if ($rep_count) {
        ...
        $text = '<p><a href="https://www.theyworkforyou.com/mp/?c=' . urlencode($va_area['name']) . '">' . sprintf(_('See %s’s voting record and speeches at TheyWorkForYou'), $name) . '</a></p>';
...
function check_area_status( $va_alone ) {
    ...
    if ($status == "boundary_changes" || $parent_status == "boundary_changes") {
        $text = _("<p>There have been boundary changes at the last election that means we can’t yet say who your representative is. We hope to get our boundary database updated as soon as we can, but at the moment we have no definite time of completion.</p>");
    } elseif ($status == "recent_election" || $parent_status == "recent_election") {
        $text = _("<p>Due to the recent election, we don’t yet have details for this representative. We generally rely on data from external sources, and will be updating as soon as we can, but at the moment we have no definite time of completion.</p>");
