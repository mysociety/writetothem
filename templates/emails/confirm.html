<th style="<?= $values['td_style'] ?> <?= $values['primary_column_style'] ?>">
  ...
  <h1 style="<?= $values['h1_style'] ?>">Please confirm your letter to <?= $values['recipient_name'] ?>.</h1>
  <p style="<?= $values['p_style'] ?>">Someone just used <a href="https://www.writetothem.com">WriteToThem.com</a> to write a letter to <?= $values['recipient_name'] ?>, from your email account.</p>
